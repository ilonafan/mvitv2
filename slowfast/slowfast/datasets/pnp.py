...
class Pnp(torch.utils.data.Dataset):
    """
    ...
    def __init__(self, cfg, mode, transform=None, num_retries=100):
        """
        ...
        logger.info("Constructing Kinetics (pnp)) {}...".format(mode))
    ...
    def _construct_loader(self):
        ...
        ), "Failed to load Kinetics (pnp) split {} from {}".format(
            self._split_idx, path_to_file
        )
        logger.info(
            "Constructing kinetics (pnp) dataloader (size: {} skip_rows {}) from {} ".format(
    ...
    def __getitem__(self, index):
        ...
        for i_try in range(self._num_retries):
            ...
            frames_decoded = frames # A list of T tentors with shape of (T, H, W, C)
            time_idx_decoded = time_idx
            
            frames_decoded_w1 = [torch.zeros((num_frames[0], 3, 224, 224))] * len(frames_decoded)  
            frames_decoded_w2 = [torch.zeros((num_frames[0], 3, 224, 224))] * len(frames_decoded)
            frames_decoded_s = [torch.zeros((num_frames[0], 3, 224, 224))] * len(frames_decoded) 
            
            frames_decoded_test = [torch.zeros((num_frames[0], 3, 224, 224))] * len(frames_decoded) 
            # frames_decoded_test = [torch.zeros((num_frames[0], 3, 256, 454))] * len(frames_decoded) 
            
            if self.mode == 'train':
                for i in range(len(frames_decoded)):
                    # Apply transformations
                    if self.transform is not None:
                        # print("The shape of frames_decoded[i] before transform is ", frames_decoded[i].shape)
                        frames_decoded[i] = frames_decoded[i].permute(0, 3, 1, 2) # Convert (T, H, W, C) to (T, C, H, W)
                        for t in range(frames_decoded[i].shape[0]):
                            frames_decoded_w1[i][t], frames_decoded_w2[i][t], frames_decoded_s[i][t] = self.transform(frames_decoded[i][t])
                        frames_decoded[i] = frames_decoded[i].permute(0, 2, 3, 1) # Convert (T, C, H, W) to (T, H, W, C)
                        frames_decoded_w1[i] = frames_decoded_w1[i].permute(0, 2, 3, 1) # Convert (T, C, H, W) to (T, H, W, C)
                        frames_decoded_w2[i] = frames_decoded_w2[i].permute(0, 2, 3, 1) # Convert (T, C, H, W) to (T, H, W, C)
                        frames_decoded_s[i] = frames_decoded_s[i].permute(0, 2, 3, 1) # Convert (T, C, H, W) to (T, H, W, C)
                ...
                for i in range(len(frames_decoded)):
                    ...
                    if self.transform is not None:
                        ...
                        frames_decoded[i] = frames_decoded[i].permute(0, 3, 1, 2) # Convert (T, H, W, C) to (T, C, H, W)
                        for t in range(frames_decoded[i].shape[0]):
                            frames_decoded_test[i][t] = self.transform(frames_decoded[i][t])
                        frames_decoded_test[i] = frames_decoded_test[i].permute(0, 2, 3, 1) # Convert (T, C, H, W) to (T, H, W, C)
            ...
            if self.mode == 'train':
                frames_decoded_train = [frames_decoded_w1, frames_decoded_w2, frames_decoded_s]
                f_out = [[None] * num_out, [None] * num_out, [None] * num_out]
                # f_out = [None] * num_out
                f_outs = [[None] * 3] * num_out
                
                for i in range(num_decode):
                    for _ in range(num_aug):
                        idx += 1
                        for e, frames_decoded_t in enumerate(frames_decoded_train):
                            f_out[e][idx] = frames_decoded_t[i].clone()
                            time_idx_out[idx] = time_idx_decoded[i, :]

                            # f_out[e][idx] = f_out[e][idx].float()
                            # f_out[e][idx] = f_out[e][idx] / 255.0

                            if (
                                self.mode in ["train"]
                                and self.cfg.DATA.SSL_COLOR_JITTER
                            ):
                                f_out[e][idx] = transform.color_jitter_video_ssl(
                                    f_out[e][idx],
                            ...
                            if self.aug and self.cfg.AUG.AA_TYPE:
                                aug_transform = create_random_augment(
                                    input_size=(f_out[e][idx].size(1), f_out[e][idx].size(2)),
                                    auto_augment=self.cfg.AUG.AA_TYPE,
                                    interpolation=self.cfg.AUG.INTERPOLATION,
                                )
                                # T H W C -> T C H W.
                                f_out[e][idx] = f_out[e][idx].permute(0, 3, 1, 2)
                                list_img = self._frame_to_list_img(f_out[e][idx])
                                list_img = aug_transform(list_img)
                                f_out[e][idx] = self._list_img_to_frames(list_img)
                                f_out[e][idx] = f_out[e][idx].permute(0, 2, 3, 1)

                            # Perform color normalization.
                            f_out[e][idx] = utils.tensor_normalize(
                                f_out[e][idx], self.cfg.DATA.MEAN, self.cfg.DATA.STD
                            )
                            

                            # T H W C -> C T H W.
                            f_out[e][idx] = f_out[e][idx].permute(3, 0, 1, 2)
                            ...
                            f_out[e][idx] = utils.spatial_sampling(
                                f_out[e][idx],
                            ...
                            if self.rand_erase:
                                ...
                                f_out[e][idx] = erase_transform(
                                    f_out[e][idx].permute(1, 0, 2, 3)
                                ).permute(1, 0, 2, 3)
                            
                            f_outs[idx][e] = f_out[e][idx]
                            
                            # f_out[e][idx] = utils.pack_pathway_output(self.cfg, f_out[e][idx])
                            # if self.cfg.AUG.GEN_MASK_LOADER:
                            #     mask = self._gen_mask()
                            #     f_out[e][idx] = f_out[e][idx] + [torch.Tensor(), mask]
                        f_outs[idx] = torch.cat((f_outs[idx][0], f_outs[idx][1], f_outs[idx][2]), dim=0)
                        
                        f_outs[idx] = utils.pack_pathway_output(self.cfg, f_outs[idx])
                        if self.cfg.AUG.GEN_MASK_LOADER:
                            mask = self._gen_mask()
                            f_outs[idx] = f_outs[idx] + [torch.Tensor(), mask]
                # print("The type of f_out is", type(f_out))
                ...
                for i in range(num_decode):
                    for _ in range(num_aug):
                        idx += 1
                        f_out[idx] = frames_decoded_test[i].clone()
                        time_idx_out[idx] = time_idx_decoded[i, :]
            ...
            # print("The type of frames is ", type(frames))
            # print("The shape of a frame is ", frames[0][0].shape)
