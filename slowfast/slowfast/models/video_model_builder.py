...
from slowfast.models.utils import (
    ...
    calc_mvit_pnp_feature_geometry,
...
@MODEL_REGISTRY.register()
class MVIT_PNP(nn.Module):
    """
    Model builder for MViTv2 with PnP modification.
    ...
    def __init__(self, cfg):
        ...
        assert cfg.DATA.TRAIN_CROP_SIZE == cfg.DATA.TEST_CROP_SIZE
        self.cfg = cfg
        pool_first = cfg.MVIT_PNP.POOL_FIRST
        # Prepare input.
        spatial_size = cfg.DATA.TRAIN_CROP_SIZE
        temporal_size = cfg.DATA.NUM_FRAMES
        in_chans = cfg.DATA.INPUT_CHANNEL_NUM[0]
        self.use_2d_patch = cfg.MVIT_PNP.PATCH_2D
        self.enable_detection = cfg.DETECTION.ENABLE
        self.enable_rev = cfg.MVIT_PNP.REV.ENABLE
        self.patch_stride = cfg.MVIT_PNP.PATCH_STRIDE
        if self.use_2d_patch:
            self.patch_stride = [1] + self.patch_stride
        self.T = cfg.DATA.NUM_FRAMES // self.patch_stride[0]
        self.H = cfg.DATA.TRAIN_CROP_SIZE // self.patch_stride[1]
        ...
        # Prepare output.
        num_classes = cfg.MODEL.NUM_CLASSES
        embed_dim = cfg.MVIT_PNP.EMBED_DIM
        # Prepare backbone
        num_heads = cfg.MVIT_PNP.NUM_HEADS
        mlp_ratio = cfg.MVIT_PNP.MLP_RATIO
        qkv_bias = cfg.MVIT_PNP.QKV_BIAS
        self.drop_rate = cfg.MVIT_PNP.DROPOUT_RATE
        depth = cfg.MVIT_PNP.DEPTH
        drop_path_rate = cfg.MVIT_PNP.DROPPATH_RATE
        layer_scale_init_value = cfg.MVIT_PNP.LAYER_SCALE_INIT_VALUE
        head_init_scale = cfg.MVIT_PNP.HEAD_INIT_SCALE
        mode = cfg.MVIT_PNP.MODE
        self.cls_embed_on = cfg.MVIT_PNP.CLS_EMBED_ON
        self.use_mean_pooling = cfg.MVIT_PNP.USE_MEAN_POOLING
        # Params for positional embedding
        self.use_abs_pos = cfg.MVIT_PNP.USE_ABS_POS
        self.use_fixed_sincos_pos = cfg.MVIT_PNP.USE_FIXED_SINCOS_POS
        self.sep_pos_embed = cfg.MVIT_PNP.SEP_POS_EMBED
        self.rel_pos_spatial = cfg.MVIT_PNP.REL_POS_SPATIAL
        self.rel_pos_temporal = cfg.MVIT_PNP.REL_POS_TEMPORAL
        if cfg.MVIT_PNP.NORM == "layernorm":
            norm_layer = partial(nn.LayerNorm, eps=1e-6)
        else:
            raise NotImplementedError("Only supports layernorm.")
        self.num_classes = num_classes
        self.patch_embed = stem_helper.PatchEmbed(
            dim_in=in_chans,
            dim_out=embed_dim,
            kernel=cfg.MVIT_PNP.PATCH_KERNEL,
            stride=cfg.MVIT_PNP.PATCH_STRIDE,
            padding=cfg.MVIT_PNP.PATCH_PADDING,
            conv_2d=self.use_2d_patch,
        )
        ...
        dim_mul, head_mul = torch.ones(depth + 1), torch.ones(depth + 1)
        for i in range(len(cfg.MVIT_PNP.DIM_MUL)):
            dim_mul[cfg.MVIT_PNP.DIM_MUL[i][0]] = cfg.MVIT_PNP.DIM_MUL[i][1]
        for i in range(len(cfg.MVIT_PNP.HEAD_MUL)):
            head_mul[cfg.MVIT_PNP.HEAD_MUL[i][0]] = cfg.MVIT_PNP.HEAD_MUL[i][1]

        pool_q = [[] for i in range(cfg.MVIT_PNP.DEPTH)]
        pool_kv = [[] for i in range(cfg.MVIT_PNP.DEPTH)]
        stride_q = [[] for i in range(cfg.MVIT_PNP.DEPTH)]
        stride_kv = [[] for i in range(cfg.MVIT_PNP.DEPTH)]

        for i in range(len(cfg.MVIT_PNP.POOL_Q_STRIDE)):
            stride_q[cfg.MVIT_PNP.POOL_Q_STRIDE[i][0]] = cfg.MVIT_PNP.POOL_Q_STRIDE[i][
                1:
            ]
            if cfg.MVIT_PNP.POOL_KVQ_KERNEL is not None:
                pool_q[cfg.MVIT_PNP.POOL_Q_STRIDE[i][0]] = cfg.MVIT_PNP.POOL_KVQ_KERNEL
            else:
                pool_q[cfg.MVIT_PNP.POOL_Q_STRIDE[i][0]] = [
                    s + 1 if s > 1 else s for s in cfg.MVIT_PNP.POOL_Q_STRIDE[i][1:]
                ]

        # If POOL_KV_STRIDE_ADAPTIVE is not None, initialize POOL_KV_STRIDE.
        if cfg.MVIT_PNP.POOL_KV_STRIDE_ADAPTIVE is not None:
            _stride_kv = cfg.MVIT_PNP.POOL_KV_STRIDE_ADAPTIVE
            cfg.MVIT_PNP.POOL_KV_STRIDE = []
            for i in range(cfg.MVIT_PNP.DEPTH):
                ...
                cfg.MVIT_PNP.POOL_KV_STRIDE.append([i] + _stride_kv)

        for i in range(len(cfg.MVIT_PNP.POOL_KV_STRIDE)):
            stride_kv[cfg.MVIT_PNP.POOL_KV_STRIDE[i][0]] = cfg.MVIT_PNP.POOL_KV_STRIDE[
                i
            ][1:]
            if cfg.MVIT_PNP.POOL_KVQ_KERNEL is not None:
                pool_kv[
                    cfg.MVIT_PNP.POOL_KV_STRIDE[i][0]
                ] = cfg.MVIT_PNP.POOL_KVQ_KERNEL
            else:
                pool_kv[cfg.MVIT_PNP.POOL_KV_STRIDE[i][0]] = [
                    s + 1 if s > 1 else s
                    for s in cfg.MVIT_PNP.POOL_KV_STRIDE[i][1:]
                ]
        ...
        self.norm_stem = norm_layer(embed_dim) if cfg.MVIT_PNP.NORM_STEM else None
        ...
        if self.enable_rev:
            ...
            if "concat" in self.cfg.MVIT_PNP.REV.RESPATH_FUSE:
                self.norm = norm_layer(2 * embed_dim)
            else:
                self.norm = norm_layer(embed_dim)
            ...
            for i in range(depth):
                num_heads = round_width(num_heads, head_mul[i])
                if cfg.MVIT_PNP.DIM_MUL_IN_ATT:
                    dim_out = round_width(
                        embed_dim,
                        dim_mul[i],
                        divisor=round_width(num_heads, head_mul[i]),
                ...
                attention_block = MultiScaleBlock(
                    ...
                    rel_pos_spatial=self.rel_pos_spatial,
                    rel_pos_temporal=self.rel_pos_temporal,
                    rel_pos_zero_init=cfg.MVIT_PNP.REL_POS_ZERO_INIT,
                    residual_pooling=cfg.MVIT_PNP.RESIDUAL_POOLING,
                    dim_mul_in_att=cfg.MVIT_PNP.DIM_MUL_IN_ATT,
                    separate_qkv=cfg.MVIT_PNP.SEPARATE_QKV,
                )
            ...
            # Head for LPN
            self.classifier_head = head_helper.TransformerBasicHead(
                2 * embed_dim
                if ("concat" in cfg.MVIT_PNP.REV.RESPATH_FUSE and self.enable_rev)
                else embed_dim,
                num_classes,
                dropout_rate=cfg.MODEL.DROPOUT_RATE,
                act_func=cfg.MODEL.HEAD_ACT,
            ...
            # Head for NPN
            self.proba_head = head_helper.NPNHead(
                2 * embed_dim
                if ("concat" in cfg.MVIT_PNP.REV.RESPATH_FUSE and self.enable_rev)
                else embed_dim,
        ...
        self.classifier_head.projection.weight.data.mul_(head_init_scale)
        self.classifier_head.projection.bias.data.mul_(head_init_scale)
        

        for _, m in self.proba_head.projection.named_modules():
            if isinstance(m, nn.Conv2d) or isinstance(m, nn.Linear):
                m.weight.data.mul_(head_init_scale)
                m.bias.data.mul_(head_init_scale)
        # self.proba_head.projection.weight.data.mul_(head_init_scale)
        # self.proba_head.projection.bias.data.mul_(head_init_scale)

        self.feat_size, self.feat_stride = calc_mvit_pnp_feature_geometry(cfg)

    def _init_weights(self, m):
        if isinstance(m, nn.Sequential):
            # print("Enter here, the type of m is ", type(m))
            for _, e in m.named_modules():
                if isinstance(e, nn.Conv2d) or isinstance(e, nn.Linear):
                    # print("Enter here, the type of e is ", type(e))
                    nn.init.kaiming_normal_(e.weight.data)
                    if e.bias is not None:
                        nn.init.constant_(e.bias.data, val=0)   
        if isinstance(m, (nn.Linear, nn.Conv2d, nn.Conv3d)):
            nn.init.trunc_normal_(m.weight, std=0.02)
            if isinstance(m, nn.Linear) and m.bias is not None:
                nn.init.constant_(m.bias, 0.02)
    ...
    def no_weight_decay(self):
        names = []
        if self.cfg.MVIT_PNP.ZERO_DECAY_POS_CLS:
            if self.use_abs_pos:
                if self.sep_pos_embed:
                    names.extend(
                        [
    ...
    def forward(self, x, bboxes=None, return_attn=False):
        ...
        if self.enable_rev:
            ...
            if self.use_mean_pooling:
                ...
                x = self.norm(x)
                x = x.mean(1)
            # print("The shape of x before fed into the heads is:", x.shape)
